...
from models.chapter_eighteen import ChapterEighteenModel
from utils.log_utils import put_error


def get_chapter_eighteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_EIGHTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_eighteen(body: ChapterEighteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_eighteen(body: ChapterEighteenPut, db: Session):
...
def delete_chapter_eighteen(body: ChapterEighteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
