...
from models.chapter_four import ChapterFourModel
from utils.log_utils import put_error


def get_chapter_four(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_FOUR WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_four(body: ChapterFourPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_four(body: ChapterFourPut, db: Session):
...
def delete_chapter_four(body: ChapterFourDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
