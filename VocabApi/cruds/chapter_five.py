...
from models.chapter_five import ChapterFiveModel
from utils.log_utils import put_error


def get_chapter_five(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_FIVE WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_five(body: ChapterFivePost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_five(body: ChapterFivePut, db: Session):
...
def delete_chapter_five(body: ChapterFiveDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
