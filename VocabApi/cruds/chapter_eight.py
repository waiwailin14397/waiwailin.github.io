...
from models.chapter_eight import ChapterEightModel
from utils.log_utils import put_error


def get_chapter_eight(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_EIGHT WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_eight(body: ChapterEightPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_eight(body: ChapterEightPut, db: Session):
...
def delete_chapter_eight(body: ChapterEightDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
