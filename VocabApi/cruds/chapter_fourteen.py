...
from models.chapter_fourteen import ChapterFourteenModel
from utils.log_utils import put_error


def get_chapter_fourteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_FOURTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_fourteen(body: ChapterFourteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_fourteen(body: ChapterFourteenPut, db: Session):
...
def delete_chapter_fourteen(body: ChapterFourteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
