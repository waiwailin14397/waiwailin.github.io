...
from models.chapter_seventeen import ChapterSeventeenModel
from utils.log_utils import put_error


def get_chapter_seventeen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_SEVENTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_seventeen(body: ChapterSeventeenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_seventeen(body: ChapterSeventeenPut, db: Session):
...
def delete_chapter_seventeen(body: ChapterSeventeenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
