...
from models.chapter_three import ChapterThreeModel
from utils.log_utils import put_error


def get_chapter_three(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_THREE WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_three(body: ChapterThreePost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_three(body: ChapterThreePut, db: Session):
...
def delete_chapter_three(body: ChapterThreeDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
