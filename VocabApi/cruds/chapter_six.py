...
from models.chapter_six import ChapterSixModel
from utils.log_utils import put_error


def get_chapter_six(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_SIX WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_six(body: ChapterSixPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_six(body: ChapterSixPut, db: Session):
...
def delete_chapter_six(body: ChapterSixDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
