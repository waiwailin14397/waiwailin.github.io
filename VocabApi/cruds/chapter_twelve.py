...
from models.chapter_twelve import ChapterTwelveModel
from utils.log_utils import put_error


def get_chapter_twelve(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_TWELVE WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_twelve(body: ChapterTwelvePost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_twelve(body: ChapterTwelvePut, db: Session):
...
def delete_chapter_twelve(body: ChapterTwelveDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
