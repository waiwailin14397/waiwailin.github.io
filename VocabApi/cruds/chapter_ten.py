...
from models.chapter_ten import ChapterTenModel
from utils.log_utils import put_error


def get_chapter_ten(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_TEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_ten(body: ChapterTenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_ten(body: ChapterTenPut, db: Session):
...
def delete_chapter_ten(body: ChapterTenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
