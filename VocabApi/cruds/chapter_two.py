...
from models.chapter_two import ChapterTwoModel
from utils.log_utils import put_error


def get_chapter_two(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_TWO WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_two(body: ChapterTwoPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_two(body: ChapterTwoPut, db: Session):
...
def delete_chapter_two(body: ChapterTwoDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
