...
from models.chapter_sixteen import ChapterSixteenModel
from utils.log_utils import put_error


def get_chapter_sixteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_SIXTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_sixteen(body: ChapterSixteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_sixteen(body: ChapterSixteenPut, db: Session):
...
def delete_chapter_sixteen(body: ChapterSixteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
