...
from models.chapter_fifteen import ChapterFifteenModel
from utils.log_utils import put_error


def get_chapter_fifteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_FIFTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_fifteen(body: ChapterFifteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_fifteen(body: ChapterFifteenPut, db: Session):
...
def delete_chapter_fifteen(body: ChapterFifteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
