...
from models.chapter_one import ChapterOneModel
from utils.log_utils import put_error


def get_chapter_one(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_ONE WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_one(body: ChapterOnePost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_one(body: ChapterOnePut, db: Session):
...
def delete_chapter_one(body: ChapterOneDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
