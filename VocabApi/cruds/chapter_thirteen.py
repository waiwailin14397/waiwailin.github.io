...
from models.chapter_thirteen import ChapterThirteenModel
from utils.log_utils import put_error


def get_chapter_thirteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_THIRTEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_thirteen(body: ChapterThirteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_thirteen(body: ChapterThirteenPut, db: Session):
...
def delete_chapter_thirteen(body: ChapterThirteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
