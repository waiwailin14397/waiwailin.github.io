...
from models.chapter_nineteen import ChapterNineteenModel
from utils.log_utils import put_error


def get_chapter_nineteen(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_NINETEEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_nineteen(body: ChapterNineteenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_nineteen(body: ChapterNineteenPut, db: Session):
...
def delete_chapter_nineteen(body: ChapterNineteenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
