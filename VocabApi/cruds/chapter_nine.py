...
from models.chapter_nine import ChapterNineModel
from utils.log_utils import put_error


def get_chapter_nine(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_NINE WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_nine(body: ChapterNinePost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_nine(body: ChapterNinePut, db: Session):
...
def delete_chapter_nine(body: ChapterNineDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
