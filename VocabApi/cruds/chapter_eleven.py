...
from models.chapter_eleven import ChapterElevenModel
from utils.log_utils import put_error


def get_chapter_eleven(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_ELEVEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_eleven(body: ChapterElevenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_eleven(body: ChapterElevenPut, db: Session):
...
def delete_chapter_eleven(body: ChapterElevenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
