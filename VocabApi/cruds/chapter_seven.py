...
from models.chapter_seven import ChapterSevenModel
from utils.log_utils import put_error


def get_chapter_seven(next_id, db: Session):
    try:
        # データ取得SQL
        query = '''
            SELECT
        ...
        query += '''
            ...
            OFFSET
                CASE WHEN :next_id = 0 THEN 0
                    ELSE
                        (SELECT COUNT(*) FROM CHAPTER_SEVEN WHERE ID < :next_id)
                    END
            LIMIT
                10
            '''
...
def post_chapter_seven(body: ChapterSevenPost, db: Session):
    try:
        # データ登録SQL
...
def put_chapter_seven(body: ChapterSevenPut, db: Session):
...
def delete_chapter_seven(body: ChapterSevenDelete, db: Session):
    try:
        # データ削除SQL
        query = '''
            DELETE FROM
